...
var beat_phase: int = 0  # 0 = lub, 1 = dub
...
func _make_heartbeat_thud(freq: float, duration: float) -> AudioStreamWAV:
	# two-thump heartbeat shape: lub-DUB
	...
	for i in samples:
		...
		# add a little sub-bass click at start
...
func _build_ui() -> void:
	...
	# tunnel canvas — drawn via _draw, sits behind everything
	...
	# gate light at center — a warm glow that grows
	gate_light = ColorRect.new()
	gate_light.color = Color(1.0, 0.9, 0.6, 0.0)
	gate_light.mouse_filter = Control.MOUSE_FILTER_IGNORE
	...
	bar_bg.color = Color(0.08, 0.08, 0.1)
...
func _draw_tunnel(canvas: Control) -> void:
	...
	# draw tunnel rings from outside in
	# as progress increases, rings compress toward center (you're getting closer to exit)
	for i in RING_COUNT:
		var t = float(i) / RING_COUNT
		# outer rings stay, inner rings collapse as progress increases
		var radius = max_radius * (1.0 - t) * (1.0 - progress * 0.5)
		if radius < 4:
			continue
		# color: dark outside, slightly lighter toward center
		var brightness = t * 0.02 + progress * 0.08
		var warm = progress * 0.10  # gets warmer (more yellow) as you near exit
		var ring_color = Color(brightness + warm, brightness + warm * 0.5, brightness)
		# draw as a thick circle outline using multiple draw_arc calls
		var thickness = max_radius * 0.06
		canvas.draw_arc(Vector2(cx, cy), radius, 0, TAU, 48, ring_color, thickness, false)

	# gate light glow at center — grows with progress
...
func _process(delta: float) -> void:
	...
	if heartbeat_timer >= heartbeat_interval:
		...
		if beat_phase == 0:
			...
			beat_phase = 1
			# schedule the "dub" (second beat) shortly after
...
func _on_hit() -> void:
	hit_player.play()
	flash_overlay.color = Color(1.0, 0.95, 0.6, 0.2)
...
func _on_win() -> void:
	...
	# tunnel fully opens — redraw at full progress
